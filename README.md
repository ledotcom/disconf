...
######乐视商城使用修改的重要说明：##################
1.增加了配置参数到数据库的加密存储，防止拖库风险；
2.支持新用户注册，每个用户可以配置多个应用，普通用户只能看到自己新建的应用配置；
3.开发用户在local环境新建配置的数据会自动同步到test、preview、online环境，且支持各自修改；
4.test 、preview 和online环境的配置只有特殊用户才有权限看到；
5.修改了注册到zk是的host+port的模式，改为appName+ip+pid的模式
########以上内容已经在2016年11月14日实现并提交到git上############
